...
function PeriodicalsPage() {
  ...
  const fetchBooks = useCallback(async () => {
    ...
    try {
      const data = await sendRequest<Array<BookData>>(
        `http://localhost:8081/api/books?knowledgeArea=${BookKnowledgeArea.Periodic}&page=1&limit=100`,
        requestOptions,
        navigate,
        logout
      );
  ...
  const debouncedSearch = useCallback(
    debounce(async (query) => {
      ...
      try {
        const data = await sendRequest<Array<BookData>>(
          `http://localhost:8081/api/books/search?knowledgeArea=${BookKnowledgeArea.Periodic}&query=${query}`,
          requestOptions,
          navigate,
          logout
        );
