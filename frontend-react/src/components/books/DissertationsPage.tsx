...
function DissertationsPage() {
  ...
  const fetchBooks = useCallback(async () => {
    ...
    try {
      const data = await sendRequest<Array<BookData>>(
        `http://localhost:8081/api/books?knowledgeArea=${BookKnowledgeArea.Dissertations}&page=1&limit=100`,
        requestOptions,
        navigate,
        logout
      );
  ...
  const debouncedSearch = useCallback(
    debounce(async (query) => {
      ...
      try {
        const data = await sendRequest<Array<BookData>>(
          `http://localhost:8081/api/books/search?knowledgeArea=${BookKnowledgeArea.Dissertations}&query=${query}`,
          requestOptions,
          navigate,
          logout
        );
      ...
      <Flex direction="column" align="center" flex="1" p="4">
        ...
        >
          <InputGroup width={{ base: "100%", md: "300px" }}>
            <Input
              placeholder={t("dissertationsPage.searchPlaceholder")}
              value={searchTerm}
