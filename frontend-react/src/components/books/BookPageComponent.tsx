...
function BooksPage() {
  ...
  const fetchBooks = useCallback(async () => {
    ...
    try {
      const data = await sendRequest<Array<BookData>>(
        `http://localhost:8081/api/books?knowledgeArea=${BookKnowledgeArea.Book}&page=1&limit=100`,
        requestOptions,
        navigate,
        logout
      );
  ...
  const debouncedSearch = useCallback(
    debounce(async (query) => {
      ...
      try {
        const data = await sendRequest<Array<BookData>>(
          `http://localhost:8081/api/books/search?knowledgeArea=${BookKnowledgeArea.Book}&query=${query}`,
          requestOptions,
          navigate,
          logout
        );
